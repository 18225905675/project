...
int pushMp4(const char *path) {
    ...
    //输出的地址
    const char *outUrl = streamOutPutUrl.UTF8String;
    ...
    //打开文件，解封文件头
    __block int ret = avformat_open_input(&ictx, inUrl, 0, NULL);
    if (ret < 0) {
        return avError(ret);
    }
    ...
    AVCodecContext *out_context = NULL;
    AVCodecContext *in_context = NULL;
    ...
    for (i = 0; i < ictx->nb_streams; i++) {
        ...
        in_context = in_stream->codec;
        out_context = out_stream->codec;
...
//        out_stream->codecpar->codec_tag = 0;
    ...
    //输入流数据的数量循环
    for (i = 0; i < ictx->nb_streams; i++) {
        if (ictx->streams[i]->codec->codec_type == FFAVMEDIA_TYPE_VIDEO) {
            videoindex = i;
            break;
        }
    }
